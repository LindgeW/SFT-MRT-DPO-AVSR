...
def tile(x: Tensor | tuple[Tensor, ...], count: int, dim=0) -> Tensor | tuple[Tensor, ...]:
    """
    Tiles x on dimension dim count times. 极简等价实现（基于PyTorch原生API）
    :param x: tensor to tile
    :param count: number of tiles
    :param dim: dimension along which the tensor is tiled
    :return: tiled tensor（输入元组则返回元组，输入张量则返回张量）
    """
    if isinstance(x, tuple):
...
def subsequent_mask(size):
    ...
    return torch.triu(torch.ones(attn_shape), 1) == 0
...
def beam_decode(
    ...
    for step in range(max_output_length):
        ...
        logits = decoder(decoder_input, encoder_output, tgt_lens=tgt_lens, src_lens=src_lens)
        ...
        # batch*k x trg_vocab
        log_probs = F.log_softmax(logits[:, -1], dim=-1).squeeze(1)
    ...
    for i, (fout, fscore) in enumerate(zip(final_outputs, final_scores)):
        if len(fout) < n_best:
            #final_outputs[i] = fout[:1] * (n_best - len(fout)) + fout
            #final_scores[i] = fscore[:1] * (n_best - len(fscore)) + fscore
            #final_outputs[i] = fout + fout[-1:] * (n_best - len(fout))
            #final_scores[i] = fscore + fscore[-1:] * (n_best - len(fscore))
            final_outputs[i] = (fout * (n_best // len(fout) + 1))[:n_best]
            final_scores[i] = (fscore * (n_best // len(fscore) + 1))[:n_best]
