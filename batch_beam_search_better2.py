...
def _build_group_slices(beam_size: int, num_groups: int) -> list[slice]:
    base_group_size = beam_size // num_groups
    remainder = beam_size % num_groups

    slices: list[slice] = []
    start = 0
    for group_idx in range(num_groups):
        current_size = base_group_size + (1 if group_idx < remainder else 0)
...
def diverse_beam_decode(
    ...
    """
    Diverse beam search with group-based hamming diversity.

    Compatibility notes:
    - Keeps the same decoder call pattern as ``beam_decode``.
    - Keeps the same core inputs and return style as ``beam_decode``.

    Search notes:
    - The beam is partitioned into ``num_groups`` groups.
    - Later groups are penalized when selecting tokens already chosen by
      earlier groups at the same decoding step.
    - Returned scores for ``n_best > 1`` follow the diversity-adjusted search
      objective so they match the ranking used during decoding.
    """
    assert beam_size > 0, "Beam size must be > 0."
    assert max_output_length > 0, "max_output_length must be > 0."
    assert n_best > 0, "n_best must be > 0."
    ...
    assert num_groups <= beam_size, "num_groups must be <= beam_size."

    if num_groups == 1 or diversity_strength <= 0:
        return beam_decode(
            decoder=decoder,
    ...
    device = encoder_output.device
    batch_size = int(src_lens.size(0))
    group_slices = _build_group_slices(beam_size, num_groups)
    ...
    for step in range(max_output_length):
        ...
        next_length = decoder_input.size(1)
        penalty = _length_penalty(next_length, alpha)
        base_ranked_scores = raw_candidate_log_probs / penalty

        beam_offsets = torch.arange(0, active_batch_size * beam_size, step=beam_size, device=device)
        token_counts = torch.zeros(
            (active_batch_size, vocab_size),
            dtype=base_ranked_scores.dtype,
            device=device,
        )
        ...
        if step + 1 == max_output_length:
            for group_slice in group_slices:
                ...
                group_base_scores = base_ranked_scores[:, group_slice, :]
                group_adjusted_scores = group_base_scores - diversity_strength * token_counts.unsqueeze(1)
                flat_group_scores = group_adjusted_scores.reshape(active_batch_size, group_width * vocab_size)

                group_top_scores, group_top_ids = flat_group_scores.topk(group_width, dim=-1)
                group_top_parents = torch.div(group_top_ids, vocab_size, rounding_mode="floor")
                group_top_tokens = group_top_ids.fmod(vocab_size)
                group_parent_indices = group_top_parents + (beam_offsets + group_slice.start).unsqueeze(1)

                group_prefixes = alive_seq.index_select(0, group_parent_indices.reshape(-1))
                group_prefixes = group_prefixes.view(active_batch_size, group_width, -1)
                group_sequences = torch.cat([group_prefixes, group_top_tokens.unsqueeze(-1)], dim=-1)

                valid_group_mask = torch.isfinite(group_top_scores)
                valid_token_increments = valid_group_mask.to(token_counts.dtype)
                token_counts.scatter_add_(1, group_top_tokens, valid_token_increments)

                for i in range(active_batch_size):
                    batch_id = int(active_batch_ids[i].item())
                    for j in range(group_width):
                        if not bool(valid_group_mask[i, j].item()):
                            continue
                        score = float(group_top_scores[i, j].item())
                        prediction = _trim_to_eos(group_sequences[i, j, 1:], eos_index).detach().clone()
                        finished[batch_id].append((score, prediction))

                    sample_done[i] = True
        else:
            group_alive_sequences: list[Tensor] = []
            group_alive_log_probs: list[Tensor] = []
            group_alive_parent_indices: list[Tensor] = []
            group_alive_scores: list[Tensor] = []

            for group_slice in group_slices:
                group_width = group_slice.stop - group_slice.start
                group_base_scores = base_ranked_scores[:, group_slice, :]
                group_raw_log_probs = raw_candidate_log_probs[:, group_slice, :]
                group_adjusted_scores = group_base_scores - diversity_strength * token_counts.unsqueeze(1)

                eos_scores = group_adjusted_scores[:, :, eos_index]
                top_finished_scores, top_finished_parents = eos_scores.topk(group_width, dim=-1)
                top_finished_raw_log_probs = group_raw_log_probs[:, :, eos_index].gather(1, top_finished_parents)
                top_finished_parent_indices = top_finished_parents + (beam_offsets + group_slice.start).unsqueeze(1)

                top_finished_prefixes = alive_seq.index_select(0, top_finished_parent_indices.reshape(-1))
                top_finished_prefixes = top_finished_prefixes.view(active_batch_size, group_width, -1)
                eos_column = torch.full(
                    (active_batch_size, group_width, 1),
                    eos_index,
                    dtype=torch.long,
                    device=device,
                )
                ...
                non_eos_scores = group_adjusted_scores.clone()
                non_eos_scores[:, :, eos_index] = float("-inf")
                flat_non_eos_scores = non_eos_scores.reshape(active_batch_size, group_width * vocab_size)

                top_alive_scores, top_alive_flat_ids = flat_non_eos_scores.topk(group_width, dim=-1)
                flat_group_raw_log_probs = group_raw_log_probs.reshape(active_batch_size, group_width * vocab_size)
                top_alive_raw_log_probs = flat_group_raw_log_probs.gather(1, top_alive_flat_ids)
                top_alive_parents = torch.div(top_alive_flat_ids, vocab_size, rounding_mode="floor")
                top_alive_tokens = top_alive_flat_ids.fmod(vocab_size)
                top_alive_parent_indices = top_alive_parents + (beam_offsets + group_slice.start).unsqueeze(1)

                top_alive_prefixes = alive_seq.index_select(0, top_alive_parent_indices.reshape(-1))
                top_alive_prefixes = top_alive_prefixes.view(active_batch_size, group_width, -1)
                top_alive_sequences = torch.cat([top_alive_prefixes, top_alive_tokens.unsqueeze(-1)], dim=-1)

                valid_alive_mask = torch.isfinite(top_alive_scores)
                alive_token_increments = valid_alive_mask.to(token_counts.dtype)
                token_counts.scatter_add_(1, top_alive_tokens, alive_token_increments)

                group_alive_sequences.append(top_alive_sequences)
                group_alive_log_probs.append(top_alive_raw_log_probs)
                group_alive_parent_indices.append(top_alive_parent_indices)
                group_alive_scores.append(top_alive_scores)

                for i in range(active_batch_size):
                    batch_id = int(active_batch_ids[i].item())
                    for j in range(group_width):
                        if not bool(torch.isfinite(top_finished_raw_log_probs[i, j]).item()):
                            continue
                        score = float(top_finished_scores[i, j].item())
                        prediction = top_finished_sequences[i, j, 1:].detach().clone()
                        finished[batch_id].append((score, prediction))

            next_alive_seq = torch.cat(group_alive_sequences, dim=1)
            next_alive_log_probs = torch.cat(group_alive_log_probs, dim=1)
            next_parent_indices = torch.cat(group_alive_parent_indices, dim=1)
            next_alive_scores = torch.cat(group_alive_scores, dim=1)

            next_batch_ids: list[int] = []
            kept_alive_seq: list[Tensor] = []
            kept_alive_log_probs: list[Tensor] = []
            kept_parent_indices: list[Tensor] = []

            for i in range(active_batch_size):
                batch_id = int(active_batch_ids[i].item())
                finished[batch_id].sort(key=lambda item: item[0], reverse=True)

                sample_alive_scores = next_alive_scores[i]
                finite_alive_scores = sample_alive_scores[torch.isfinite(sample_alive_scores)]

                if len(finished[batch_id]) >= n_best:
                    nth_finished_score = finished[batch_id][n_best - 1][0]
                    if finite_alive_scores.numel() == 0 or float(finite_alive_scores.max().item()) <= nth_finished_score:
                        sample_done[i] = True
                        continue

                if finite_alive_scores.numel() == 0:
                    sample_done[i] = True
                    continue

                kept_alive_seq.append(next_alive_seq[i])
                kept_alive_log_probs.append(next_alive_log_probs[i])
                kept_parent_indices.append(next_parent_indices[i])
                next_batch_ids.append(batch_id)

        if bool(sample_done.all()):
            break

        alive_seq = torch.cat(kept_alive_seq, dim=0)
        alive_log_probs = torch.stack(kept_alive_log_probs, dim=0)
        select_indices = torch.cat(kept_parent_indices, dim=0)
        encoder_output = encoder_output.index_select(0, select_indices)
        src_lens = src_lens.index_select(0, select_indices)
        active_batch_ids = torch.tensor(next_batch_ids, dtype=torch.long, device=device)
...
beam_decode_diverse = diverse_beam_decode
